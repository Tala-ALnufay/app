...
app.post("/api/register", async (request, response) => {
  ...
  try {
    const existing = await pool.query("SELECT * FROM rider WHERE email = $1", [email]);
    if (existing.rows.length > 0) {
      return response.status(400).json({ message: "Email already exists" });//400 error Request
    }

    await pool.query("INSERT INTO rider (name, email, password,phone) VALUES ($1, $2, $3,$4)", [name, email, hashedPassword,phone]);
...
app.post("/api/login", async (request, response) => {
  ...
  try {
    const result = await pool.query("SELECT * FROM rider WHERE email = $1", [email]);
...
app.get("/api/user", authenticateToken,authorizeType(["admin"]),async (request, response) => { 
  try {
const result = await pool.query("SELECT id, name, email FROM rider");
...
app.put("/api/user",authenticateToken, async (request, response) => {           
  ...
  try { 
    const checkUser = await pool.query("SELECT * FROM rider WHERE id = $1", [userId]);
    if (checkUser.rows.length === 0) {
      return response.status(404).json({ message: "User not found" }); // 404 Not Found
    }
    const result = await pool.query(
      "UPDATE rider SET name = $1, email = $2,phone = $3 WHERE id = $4",
...
app.delete("/api/user",authenticateToken,authorizeType(["admin"]), async (request, response) => {
  ...
  try {
   ...
   const result = await pool.query("DELETE FROM rider WHERE id = $1", [userId]);
